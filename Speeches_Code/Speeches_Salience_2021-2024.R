...
usa_speeches_whole = usa_speeches_whole %>%ungroup() %>%mutate(date=str_extract(date,"....")) 
...
#usa_speeches_whole %>% group_by(date) %>% summarise(salience = mean(USA)) %>% export("salience_overall_2021-2024.xlsx")
...
usa_speeches_whole %>%
  ...
  mutate(salience = (n/n_sentences)*100) %>% filter(USA==T) %>% select(-c(USA,n)) #%>% export("Party_Salience_2021-2024.xlsx")
...
usa_speeches_whole %>%left_join(political_parties,by="political_party") %>% ungroup() %>% 
  ...
  mutate(salience = (n.x/n.y)*100) %>% filter(USA.x==T) #%>% export("Ideology_salience_2021-2024.xlsx")
