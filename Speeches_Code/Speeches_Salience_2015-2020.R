...
speeches_three = read_csv("Parliamentary_Speeches_2015-2020.csv")
...
speeches_three$sentence = chartr("ή ά ό ύ ώ έ ί ϊ ΐ ϋ", "η α ο υ ω ε ι ι ι υ", speeches_three$sentence)
usa_speeches = speeches_three %>% mutate(USA=str_detect(sentence,"ΗΠΑ |Η\\.Π\\.Α\\. |ηπα |η\\.π\\.α\\. |ατλαντικη|Ατλαντικη| αμερικ| Αμερικ| νατο |ΝΑΤΟ|Ν\\.Α\\.Τ\\.Ο\\.|Ηνωμενες|ηνωμενες|Ηνωμενων π|ηνωμενων π")) %>% filter(USA==T)


not_usa_speeches = speeches_three%>% anti_join(usa_speeches,by=c("sentence","member_name"))
...
#usa_speeches_whole %>% group_by(date) %>% summarise(salience = mean(USA)) %>% export("salience_overall_2015-2020.xlsx")
...
usa_speeches_whole %>%
  ...
  mutate(salience = (n/n_sentences)*100) %>% filter(USA==T) %>% select(-c(USA,n)) #%>% export("Party_Salience_2015-2020.xlsx")
...
usa_speeches_whole %>%left_join(political_parties,by="political_party") %>% ungroup() %>% 
  ...
  mutate(salience = (n.x/n.y)*100) %>% filter(USA.x==T) #%>% export("Ideology_salience_2015-2020.xlsx")
